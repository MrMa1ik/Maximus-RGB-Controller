export function Name() { return "Maximus RGB Controller (Raw USB)"; }
...
export function ImageUrl() { return "https://raw.githubusercontent.com/MrMa1ik/Maximus-RGB-Controller/refs/heads/main/Assets/Maximis_RGB_controller.png"; }
...
export function Render() {
	...
	// Dynamic Frame Rate Target Logic
	let targetFps = 60;
	if (totalActiveLeds <= 250) {
		targetFps = 90; // Higher limit for light LED loads
	} else if (totalActiveLeds <= 500) {
		targetFps = 60; // Standard 60 FPS
	} else if (totalActiveLeds <= 750) {
		targetFps = 45; // Smooth scale down for medium loads
	} else {
		targetFps = 35; // Prevents USB congestion on large loads
	}

	// Only apply target changes on state boundaries to prevent frame stutters
	if (targetFps !== currentFpsTarget) {
		currentFpsTarget = targetFps;
		device.setFrameRateTarget(targetFps);
	}
...
function ApplyHardwareEffect() {
	...
	const state = effectId + ":" + brightness + ":" + speed + ":" + color.join(",");
	...
	for (let channel = 0; channel < ChannelCount; channel++) {
		...
		// FIXED: Query the layout to find the configured count of added components on this channel
		let ledCount = ChannelLed; // Default fallback to 250
		if (componentChannel !== undefined && componentChannel !== null) {
			ledCount = componentChannel.LedCount();
		}

		// FIXED: Send the active layout count so the Pico saves the correct length
