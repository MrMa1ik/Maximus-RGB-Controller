...
void processCommand(const char* command) {
    if (strncmp(command, "BRIGHTNESS ", 11) == 0) {
        ...
        // FIXED: Only clear output and refresh if the count is actually different.
        // This completely eliminates the one-time black blink/flash when saving or applying.
        if (settingsGetLedCount(output) != value) {
    ...
    else if (strncmp(command, "COLOR2 ", 7) == 0) { // ADDED command parser for custom secondary colors
        int output, red, green, blue;
        if (!parseOutputColor(command + 7, &output, &red, &green, &blue)) return;
        red   = (red   < 0) ? 0 : (red   > 255 ? 255 : red);
        green = (green < 0) ? 0 : (green > 255 ? 255 : green);
        blue  = (blue  < 0) ? 0 : (blue  > 255 ? 255 : blue);

        settingsSetEffectColor2(output, (uint8_t)red, (uint8_t)green, (uint8_t)blue);
        currentMode = CONTROLLER_STARTUP;
    }
